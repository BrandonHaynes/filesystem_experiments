...
### File System
By far the easiest method to store and retrieve video data is to just use ordinary file system operations.  Reading and writing binary files is trivial, so if the file system is comparable to the database systems, it would have been the clear choice in terms of ease of use.
...
Finally, we compared the throughput, in tiles per second, of the different systems. As you can see, aside from the SciDB and padded file system method, all throughputs were approximately the same.  We think any differences between those bars are just due to random amazon AWS issues.  Those methods all used between 95 and 100 percent of our IO bandwidth, suggesting that for those methods, our efficiency was consistently IO bound and their similar throughputs make sense. 
