...
Recent advances in virtual reality (VR) have prompted the development of new applications ranging from gaming and entertainment to education and scientific visualization.  Spherical panoramic VR videos (a.k.a. *360 videos*) have become increasingly popular in many of these applications.  360 videos allow the user to view a spherical video scene from any angle.  Such videos provide a truly immersive experience, as they allow the user to observe the scene as they would observe their surroundings in their everyday life.
...
We elected to leave the tiles as HEVC-encoded video, as the size of the unencoded video is 250%+ larger than its encoded counterpart.  An alternate format is HEVC-encoded video MPEG-4 pre-muxed as DASH-compatible fragments, which would allow us to use the initialization segment as an index and potentially improving query performance.  However, investigating that experiment was beyond the scope of this project.
...
### File System
By far the easiest method to store and retrieve video data is to just use ordinary file system operations.  Reading and writing binary files is trivial, so if the file system is comparable to the database systems, it would be the clear choice in terms of ease of use.
...
![SciDB Architecture](images/scidb.png)

### TileDB
TileDB is a new array-based database management system that is currently used by the Intel Health and Life Sciences Group to process genomic data.  Papadopoulos et al. developed TileDB to be optimized for both dense and sparse arrays.  It focuses on fast storage and retrieval of array-based data.  In their [paper](https://doi.org/10.14778/3025111.3025117), they compare the performance of loading and updating data in TileDB to that of SciDB, and conclude that TileDB is faster for both dense and sparse arrays.

![TileDB Architecture](images/tiledb.png)
...
3. Throughput of 9mbps high-quality tiles
...
### Experiment 1:
The first experiment investigated reading tiles of different bitrates.  For the graphs below, the y axis corresponds to the time in seconds, and the x axis corresponds to the number of files at each of two bitrates (50kbps and 9mbps).
...
Aside from the SciDB and padded file system method, all throughputs were approximately equal.  Any differences between those bars are likely due to random amazon AWS variations.  Those methods all used between 95 and 100 percent of our IO bandwidth, suggesting that for those methods, efficiency was consistently IO bound.  Their similar throughputs therefore make sense. 


## Conclusions 
We found that SciDB is less efficient than either TileDB or the file system for reading, writing, and throughput, so should not be used for storing and retrieving video data.  TileDB using overflow pages outperformed native TileDB storage for writing and reading.  The file system using tiles stored in individual files consistently outperformed the other file-system-based methods, and was approximately equal to TileDB with overflow pages for everything other than reading.
